  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
        <div>
          <h3 className="text-lg font-semibold text-gray-800">收入分配预设</h3>
      ...
      {incomePresets.length > 0 && editingId === null && (
        <ul className="space-y-2 mb-4">
          {incomePresets.map((preset) => (
            <li
              key={preset.id}
              className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-4 py-3 bg-gray-50 rounded-xl border border-gray-100"
            >
              <div>
              ...
              <div className="flex items-center space-x-2 shrink-0">
                <button
                  ...
                  className="flex items-center space-x-1 text-sm text-blue-600 hover:text-blue-800 px-2 py-1"
                >
                  <Pencil size={14} />
                  <span>编辑</span>
                </button>
                <button
                  ...
                  className="flex items-center space-x-1 text-sm text-rose-600 hover:text-rose-800 px-2 py-1"
                >
                  <Trash2 size={14} />
                  <span>删除</span>
                </button>
      ...
      {editingId !== null && (
        <div className="border border-blue-200 bg-blue-50/50 rounded-xl p-4 space-y-4">
          ...
          <div className="space-y-2">
            <span className="text-xs font-medium text-gray-600">各池比例 (%)</span>
            {draftRows.map((row, index) => (
              <div key={index} className="flex items-center space-x-2">
                ...
                <input
                  ...
                  className="w-28 px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                />
            ...
            <p
              className={cn(
                'text-sm',
                Math.abs(sumPercent - 100) > 0.01 ? 'text-rose-600' : 'text-emerald-600'
              )}
            >
